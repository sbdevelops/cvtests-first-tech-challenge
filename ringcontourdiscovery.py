...
def inverse_color(inp):
    arr = ~inp
    return [int(x) for x in arr] if isinstance(arr, Iterable) else int(arr)


def inverse_color_at_point(img, pt):
    inp = img[pt[1], pt[0]]
    return inverse_color(inp)
...
img = cv2.imread('images/rings_B0.jpg')
